...
Well, you can use `Alpine.reactive` to make your own reactive objects! This is useful mainly for plugins that might want to hook into reactively without waiting for values to exist in a component.
...
```html
<button type="button" @click="console.log(Alpine.raw(cartItems))"></button>
```

## `Alpine.addRootSelector(selectorCallback: () => string): void`
...
### Bonus: `Alpine.addInitSelector(selectorCallback: () => string): void`
...
## `Alpine.addScopeToNode(node: Element, data: Record<string, unknown>, referenceNode?: Element): () => void`
...
### Bonus: `Alpine.$data(node: Element): void`
...
### `Alpine.throttle(callback: Function, wait: number): Function`
...
### `Alpine.debounce(callback: Function, wait: number): Function`
