...
Global configuration, naturally, applies to all toasts and toasters in your application. This is done by passing a configuration object to the `Toasts` plugin when you add it to Alpine. Accepts the full range of configuration options.
...
Accepts the full range of configuration options.
...
Accepts Toast configuration options only.

```html
<button type="button"
  @click="$toast.success('You are so pretty', { dismissible: false, duration: 10_000 })">
  What are you?
</button>
```
